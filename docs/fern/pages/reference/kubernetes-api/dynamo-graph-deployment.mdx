...
<ParamField path="restart" type="Restart">
  Restart policy for the graph. Change `restart.id` to trigger a restart. For the operational workflow — prerequisites, `kubectl patch` commands, progress fields, and recovery — see [Restart a DynamoGraphDeployment](../../kubernetes/operations/restart-dynamo-graph-deployment.mdx).
</ParamField>
